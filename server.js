...
servidor.get('/usuarios', async (request, reply) => {
    try {
        const resultado = await sql.query(
            `select id, nome_completo, email, criado_em from usuario order by criado_em desc;`
        )

        return { sucesso: true, usuarios: resultado.rows }

    } catch (erro) {
        console.error(erro)
        reply.status(500)
        return { sucesso: false, erros: ['erro ao buscar usuários'] }
    }
})
